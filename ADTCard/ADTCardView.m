...
@property (nonatomic, assign) ADTCardViewDragMode dragMode;
@property (nonatomic, assign) BOOL isMoving;

@end

@implementation ADTCardView
...
- (void)setDataSource:(id<ADTCardViewDataSource>)dataSource
{
...
{
    _dragMode = ADTCardViewDragByHand;
    _isMoving = YES;
    //禁用其他卡片
...
//创建卡片容器
- (__kindof ADTCardViewItem *)creatCardContainView:(NSInteger)index
{
    if ([self.dataSource respondsToSelector:@selector(handleView:itemForIndex:)]) {
        
        ADTCardViewItem *item = [self.dataSource handleView:self itemForIndex:index];
...
- (void)endDrag
{
    ...
    if (self.firstView.center.x <= [UIScreen mainScreen].bounds.size.width*0.2 &&_enable) {
        ...
        if ([self.delegate respondsToSelector:@selector(handleView:effectiveDragDirection:itemIndex: dragMode:)]) {
            [self.delegate handleView:self effectiveDragDirection:ADTCardViewDragDirectionLeftType itemIndex:_dragIndex dragMode:_dragMode];
        }
        _dragIndex ++;
        ...
        if ([self.delegate respondsToSelector:@selector(handleView:effectiveDragDirection:itemIndex:dragMode:)]) {
            [self.delegate handleView:self effectiveDragDirection:ADTCardViewDragDirectionRightType itemIndex:_dragIndex dragMode:_dragMode];
        }
        //滑动结束
...
- (void)excuteSlide:(ADTCardViewDragDirectionType)direction
{
    ...
    _isMoving = YES;
    _dragMode = ADTCardViewDragByClick;
    ...
    if (!_enable) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            if (direction == ADTCardViewDragDirectionLeftType) {
                transformX = - 150;
            }else if (direction == ADTCardViewDragDirectionRightType){
                transformX =   150;
            }
...
- (__kindof ADTCardViewItem *)dequeueReusableItemWithIdentifier:(NSString *)identifier
{
    ...
    ADTCardViewItem *item = [[class alloc]initWithReuseIdentifier:identifier];
    return item;
}
//
...
- (ADTCardViewDragDirectionType)direction
{
    ADTCardViewDragDirectionType direction = 0;
    if (self.firstView.center.x >[UIScreen mainScreen].bounds.size.width*0.5) {
        direction = ADTCardViewDragDirectionRightType;
    }else{
        direction = ADTCardViewDragDirectionLeftType;
    }
    return direction;
}
